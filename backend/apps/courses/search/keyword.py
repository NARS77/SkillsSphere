...
class DatabaseKeywordSearchEngine(BaseSearchEngine):
    ...
    def search(self, query: str, user=None, **kwargs) -> dict:
        ...
        courses = Course.objects.filter(
            Q(title__icontains=query) | Q(short_description__icontains=query), status=Course.Status.PUBLISHED
        )[:10]

        # 2. Search Lessons
        lessons_qs = Lesson.objects.filter(
            Q(title__icontains=query) | Q(description__icontains=query), status=Lesson.Status.PUBLISHED
        )

        # 3. Search Discussions
        discussions_qs = DiscussionThread.objects.filter(Q(title__icontains=query) | Q(content__icontains=query))
        ...
        if not user or user.is_anonymous:
            ...
            discussions_qs = discussions_qs.filter(
                Q(course__enrollments__student=user, course__enrollments__is_active=True) | Q(course__instructor=user)
            ).distinct()
