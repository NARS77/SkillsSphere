...
class PublicCourseViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return repo.get_published_courses(filters=filters, search_query=search_query, sort_by=sort_by)
...
class InstructorCourseViewSet(viewsets.ModelViewSet):
    ...
    def students(self, request, pk=None):
        ...
        total_lessons = (
            ...
            .filter(lessons__status="PUBLISHED")
            .count()
        )
        ...
        for enroll in enrollments:
            ...
            completed_count = student.progress_records.filter(
                course_id=course.id, is_completed=True, lesson__status="PUBLISHED"
            ).count()
    ...
    def analytics(self, request, pk=None):
        ...
        for enroll in enrollments:
            completed_count = enroll.student.progress_records.filter(
                course_id=course.id, is_completed=True, lesson__status="PUBLISHED"
            ).count()
        ...
        avg_completion = (
            round(sum(completion_percentages) / len(completion_percentages), 1) if completion_percentages else 0.0
        )

        # 3. Most watched lesson
        lesson_ids = course.sections.prefetch_related("lessons").values_list("lessons__id", flat=True)
        most_watched = (
            WatchHistory.objects.filter(lesson_id__in=lesson_ids)
            .values("lesson__title")
            .annotate(total_time=Sum("watch_time"))
            .order_by("-total_time")
            .first()
        )
        ...
        # 4. Drop-off lesson
        drop_off = (
            WatchHistory.objects.filter(lesson_id__in=lesson_ids, completion_percentage__lt=90.0)
            .values("lesson__title")
