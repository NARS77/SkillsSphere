...
class MessagingService:
    ...
    def get_or_create_conversation(student, instructor_id):
        ...
        conversation, created = Conversation.objects.get_or_create(student=student, instructor=instructor)
        return conversation
    ...
    def send_message(sender, conversation_id, content, attachment=None):
        ...
        message = Message.objects.create(
            conversation=conversation, sender=sender, content=content, attachment=attachment
        )
    ...
    def mark_as_read(user, conversation_id):
        conversation = get_object_or_404(Conversation, id=conversation_id)
        unread_messages = Message.objects.filter(conversation=conversation, is_read=False).exclude(sender=user)
