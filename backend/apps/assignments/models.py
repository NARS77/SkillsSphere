...
class Assignment(BaseBusinessModel):
    ...
    lesson = models.OneToOneField(
        "curriculum.Lesson", null=True, blank=True, on_delete=models.SET_NULL, related_name="assignment"
    )
    ...
    rubric = models.JSONField(
        null=True, blank=True, help_text="List of rubric criteria, e.g. [{'criteria': 'Code Quality', 'points': 50}]"
    )
    late_submission_rule = models.CharField(max_length=20, choices=LateRules.choices, default=LateRules.ALLOWED)
    late_penalty_percentage = models.DecimalField(
        max_digits=5, decimal_places=2, default=0.0, help_text="Percentage deducted per day late"
    )
...
class AssignmentSubmission(BaseBusinessModel):
    ...
    rubric_scoring = models.JSONField(
        null=True, blank=True, help_text="Dict mapping criteria description to awarded points"
    )
