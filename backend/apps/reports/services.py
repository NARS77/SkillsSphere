...
class ReportService:
    ...
    def get_instructor_revenue_stats(instructor):
        ...
        # 2. Monthly earnings
        now = timezone.now()
        monthly_items = paid_items.filter(
        ...
        paid_payouts_total = Payout.objects.filter(
        ...
        best_sellers = Course.objects.filter(instructor=instructor).annotate(
        ...
        daily_trends = paid_items.filter(
