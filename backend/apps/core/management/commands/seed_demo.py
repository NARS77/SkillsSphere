...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        admin1 = User.objects.create_superuser(
            username='admin_demo',
            email='admin@skillsphere.demo',
            password='Demo@123',
            role='ADMIN',
            is_verified=True
        )
        admin2 = User.objects.create_superuser(
            username='admin2_demo',
            email='admin2@skillsphere.demo',
            password='Demo@123',
            role='ADMIN',
            is_verified=True
        )
        ...
        instructors = []
        instructor_names = [
            ("Dr. Angela", "Yu"), ("Colt", "Steele"), ("Stephen", "Grider"), 
            ("Maximilian", "Schwarzmüller"), ("Jonas", "Schmedtmann"), ("Brad", "Traversy"), 
            ("Mosh", "Hamedani"), ("Kent", "C. Dodds"), ("Dan", "Abramov"), 
            ("Cory", "House"), ("Angela", "Carter")
        ]
        ...
        student_names = [
            ("Rahul", "Sharma"), ("Sarah", "Johnson"), ("Alex", "Kim"), 
            ("Emily", "Carter"), ("Ahmed", "Hassan"), ("Rohan", "Gupta"),
            ("Jessica", "Taylor"), ("Michael", "Chen"), ("Siddharth", "Patel"),
            ("Aisha", "Bello"), ("Kenji", "Sato"), ("Chloe", "Lefevre"),
            ("Carlos", "Gomez"), ("David", "Miller"), ("John", "Doe"),
            ("Maria", "Rodriguez"), ("James", "Wilson"), ("Emma", "Davis")
        ]
        ...
        courses = []
        course_titles = [
            ("React for Beginners", "Learn to build high-performance web applications using React hooks, components, and state management.", 0),
            ("Django REST API Masterclass", "Build production-ready, scalable REST APIs using Django, DRF, and JWT authentication.", 0),
            ("Docker Essentials", "Master containerizing applications, writing Dockerfiles, volumes, networks, and container registries.", 0),
            ("Machine Learning Fundamentals", "Implement regression, classification, clustering, neural networks, and decision trees using Python.", 1),
            ("Cloud Computing with AWS", "Explore AWS EC2, S3, RDS, Lambda functions, and serverless application designs.", 1),
            ("Advanced TypeScript", "Master advanced TS types, decorators, generic programming, utilities, and configuration mappings.", 0),
            ("UI/UX Design Principles", "Learn color theory, layout grids, auto-layout, design tokens, and components in Figma.", 2),
            ("Full-Stack React & Django Monoliths", "Combine Vite, React, and Django REST framework for complete monolith deployments.", 0),
            ("Mastering LLMs & Vector Databases", "Implement vector embeddings, RAG architectures, and custom AI agents with LangChain.", 1),
            ("System Design & Scalable Architectures", "Understand horizontal scaling, caching strategies, rate limiters, and message queues.", 0),
            ("NextJS Production-Ready Frameworks", "Deep dive into App Router, Server Actions, Server Components, and static site caching.", 0),
            ("Advanced SQL & Database Tunings", "Optimize queries, structure indexing, execute partitioning, and manage transactions.", 1),
            ("Data Structures & Algorithms in Python", "Master dynamic programming, heap structures, tree graph travels, and complexities.", 1),
            ("TailwindCSS Premium Web Design", "Build responsive landing pages using sleek minimal tokens and grid frameworks.", 2),
            ("iOS Swift Development with SwiftUI", "Create production-ready native applications with reactive state and animations.", 3),
            ("Introduction to Generative AI Systems", "Comprehensive foundations on NLP, transformer networks, and custom model tunings.", 1),
            ("Redis Caching & Celery Workers", "Implement asynchronous background queues and in-memory caches in Django.", 0),
            ("Mobile App Development with Flutter", "Build robust cross-platform mobile apps for Android and iOS using Dart.", 3),
            ("Product Management for SaaS startups", "Determine product-market fit, layout user journeys, and define features.", 2),
            ("React Native Immersive Workspaces", "Develop hybrid mobile apps, hook native modules, and manage layouts.", 3),
            ("Financial Analytics & NumPy Charts", "Analyze time-series structures, calculate yields, and plot data frames.", 1)
        ]
        ...
        for course in courses:
            ...
            for s in range(1, 4):
                section = Section.objects.create(
                    course=course,
                    title=f"Module {s}: Core Architecture Foundations",
                    order=s
                )
                ...
                for l in range(1, 4):
                    ...
                    LessonResource.objects.create(
                        lesson=lesson,
                        title="Lecture Slides & Docs",
                        file="resources/slides.pdf"
                    )
        ...
        for idx, lesson in enumerate(lessons_pool):
            if idx % 3 == 0:
                ...
                for q in range(1, 4):
                    ...
                    AnswerOption.objects.create(question=question, text="Better separation of concerns", is_correct=True, order=1)
                    AnswerOption.objects.create(question=question, text="Increased database speed", is_correct=False, order=2)
        ...
        for i in range(12):
            ...
            for m in range(9):
                ...
                Message.objects.create(
                    conversation=conv,
                    sender=sender,
                    content="Hello! I had a quick question regarding custom Django middleware configuration." if (m % 2 == 0) else "Sure! Make sure you override both __init__ and __call__ methods and call the get_response delegate.",
                    is_read=True,
        ...
        ai_topics = [
            ("React Hooks", [
                ...
                {"role": "assistant", "content": "You can prevent unnecessary rerenders by utilizing: 1. React.memo to memoize functional components, 2. useCallback to memoize callback functions, and 3. useMemo to cache computed properties across renders."}
            ]),
            ("Python Generators", [
                {"role": "user", "content": "What is the difference between yield and return in Python?"},
                {"role": "assistant", "content": "The 'return' statement terminates a function completely and returns a value. In contrast, 'yield' pauses the function, saves its state, and returns a generator iterator value, allowing you to resume execution later."}
            ]),
            ("Docker Networking", [
                {"role": "user", "content": "How do Docker containers communicate with each other?"},
                {"role": "assistant", "content": "Docker containers communicate by joining the same user-defined bridge network. Once joined, they can reference each other using container names as hostnames via Docker's built-in DNS."}
            ]),
            ("SQL Optimization", [
                {"role": "user", "content": "How do I speed up a slow query joining three tables?"},
                {"role": "assistant", "content": "To optimize a slow join: 1. Run EXPLAIN to locate bottlenecks, 2. Ensure foreign keys have indexes, 3. Retrieve only necessary columns rather than SELECT *, and 4. Denormalize highly critical paths if needed."}
            ]),
            ("REST APIs", [
                {"role": "user", "content": "What makes an API RESTful?"},
                {"role": "assistant", "content": "An API is RESTful when it complies with architectural constraints: 1. Client-Server separation, 2. Statelessness, 3. Cacheability, 4. Uniform interface using HTTP verbs (GET, POST, PUT, DELETE), and 5. Layered system organization."}
            ])
        ]
        ...
        notification_templates = [
            ...
            ("Certificate Earned!", "Congratulations! You have completed all course requirements and earned your certificate.", "CERTIFICATE_EARNED"),
            ("New Discussion Reply", "An instructor replied to your comment in Section 2.", "NEW_DISCUSSION"),
            ("Course Materials Updated", "New learning resources and slides were added to the course workspace.", "COURSE_UPDATED"),
        ...
        badge_data = [
            ...
            ("Certified Graduate", "Completed all sections and generated a cryptographic completion certificate.", "award", "COURSE")
        ]
