...
class CertificateService:
    ...
    def _generate_certificate_assets(certificate):
        ...
        draw_centered_text(580, "for successfully completing all requirements for the course", subtitle_font, fill="#64748b")
        draw_centered_text(650, f"\"{certificate.course.title}\"", name_font, fill="#1e3a8a")

        # Instructor & Date signatures
        instructor_name = f"{certificate.course.instructor.first_name} {certificate.course.instructor.last_name}".strip() or certificate.course.instructor.username
