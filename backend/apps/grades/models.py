...
class GradebookEntry(BaseBusinessModel):
    enrollment = models.OneToOneField(
        "enrollments.Enrollment", on_delete=models.CASCADE, related_name="gradebook_entry"
    )
    ...
    quiz_average = models.DecimalField(
        max_digits=5, decimal_places=2, default=0.0, help_text="Average score percentage of all quizzes"
    )
    assignment_average = models.DecimalField(
        max_digits=5, decimal_places=2, default=0.0, help_text="Average score percentage of all assignments"
    )

    overall_score = models.DecimalField(
        max_digits=5, decimal_places=2, default=0.0, help_text="Weighted overall percentage score"
    )
