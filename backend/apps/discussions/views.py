...
from .serializers import (
    DiscussionThreadSerializer, DiscussionThreadDetailSerializer, 
...
class DiscussionThreadViewSet(viewsets.ModelViewSet):
    ...
    def pin(self, request, pk=None):
        ...
        return Response({'message': f"Thread {'pinned' if pin_status else 'unpinned'} successfully."}, status=status.HTTP_200_OK)

    @action(detail=True, methods=['post'], url_path='lock')
    def lock(self, request, pk=None):
        lock_status = request.data.get('lock', True)
        DiscussionService.lock_thread(request.user, pk, lock_status)
        return Response({'message': f"Thread {'locked' if lock_status else 'unlocked'} successfully."}, status=status.HTTP_200_OK)
...
class DiscussionReplyViewSet(viewsets.ModelViewSet):
    ...
    def accept(self, request, pk=None):
        ...
        return Response({'message': f"Reply {'accepted' if accept_status else 'unaccepted'} successfully."}, status=status.HTTP_200_OK)
