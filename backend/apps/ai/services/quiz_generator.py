...
class AIQuizGenerator:
    ...
    def generate_quiz_questions(
        self, user, course, lesson_title: str, lesson_content: str, difficulty: str = "medium"
    ) -> list:
        from apps.ai.prompts import PromptRegistry

        template = PromptRegistry.get_prompt("quiz_generator")
        prompt = template.render(lesson_title=lesson_title, lesson_content=lesson_content[:3000], difficulty=difficulty)
