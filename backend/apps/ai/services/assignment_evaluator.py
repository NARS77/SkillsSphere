...
class AIAssignmentEvaluator:
    ...
    def evaluate_submission(
        self, user, course, assignment_title: str, assignment_desc: str, submission_text: str
    ) -> str:
        from apps.ai.prompts import PromptRegistry

        template = PromptRegistry.get_prompt("assignment_evaluator")
        prompt = template.render(
            assignment_title=assignment_title, assignment_desc=assignment_desc, submission_text=submission_text
        )
