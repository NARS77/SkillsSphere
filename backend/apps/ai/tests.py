...
class AITestCase(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            email="student@example.com", username="student", password="Password123!", role="STUDENT"
        )
        self.client.force_authenticate(user=self.user)

        # Setup instructor user
        self.instructor = User.objects.create_user(
            email="instructor@example.com", username="instructor", password="Password123!", role="INSTRUCTOR"
        )
