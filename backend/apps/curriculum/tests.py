...
class CurriculumAPITests(APITestCase):
    def setUp(self):
        ...
        self.student = User.objects.create_user(
            username="student1", email="student1@skillsphere.com", password="Password123!", role=User.Role.STUDENT
        )

        # Create course
        self.course = Course.objects.create(
            title="React Advanced", instructor=self.instructor, status=Course.Status.DRAFT
        )
    ...
    def test_instructor_can_create_section(self):
        ...
        data = {"course_id": str(self.course.id), "title": "Chapter 1: Getting Started"}
    ...
    def test_student_blocked_from_curriculum_management(self):
        ...
        data = {"course_id": str(self.course.id), "title": "Hacked Section"}
        response = self.client.post(self.section_list_url, data, format="json")
    ...
    def test_reorder_sections_success(self):
        ...
        data = {"course_id": str(self.course.id), "section_ids": [str(s2.id), str(s1.id)]}  # Invert order
    ...
    def test_reorder_lessons_cross_sections(self):
        ...
        Lesson.objects.create(section=sec1, title="Lesson 2", order=2, duration=10, status=Lesson.Status.PUBLISHED)

        reorder_url = reverse("lesson-reorder")

        # Move lesson1 to sec2
        data = {"section_id": str(sec2.id), "lesson_ids": [str(lesson1.id)]}
        response = self.client.post(reorder_url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
