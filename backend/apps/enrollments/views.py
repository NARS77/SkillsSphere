...
class LearningProgressViewSet(viewsets.ViewSet):
    ...
    def position(self, request, pk=None):
        ...
        service = LearningProgressService()
        history = service.save_watch_position(request.user, pk, seconds_int, device=device, watch_time=watch_time_int)
    ...
    def classroom(self, request, slug=None):
        ...
        progress_map = {
            p.lesson_id: p.is_completed for p in UserProgress.objects.filter(student=request.user, course_id=course.id)
        }

        # Watch history map (lesson_id -> last_position)
        watch_map = {w.lesson_id: w.last_position for w in WatchHistory.objects.filter(student=request.user)}

        # Bookmarks set
        bookmarks_set = set(Bookmark.objects.filter(student=request.user).values_list("lesson_id", flat=True))
