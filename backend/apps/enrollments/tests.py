...
class EnrollmentsAPITests(APITestCase):
    def setUp(self):
        ...
        self.student = User.objects.create_user(
            username="student2", email="student2@skillsphere.com", password="Password123!", role=User.Role.STUDENT
        )

        # Create published and draft courses
        self.published_course = Course.objects.create(
            title="Published React Course", instructor=self.instructor, status=Course.Status.PUBLISHED
        )
        self.draft_course = Course.objects.create(
            title="Draft React Course", instructor=self.instructor, status=Course.Status.DRAFT
        )
        ...
        self.lesson = Lesson.objects.create(
            section=self.section, title="Lesson 1: Intro", duration=10, status=Lesson.Status.PUBLISHED
        )
    ...
    def test_enroll_published_course_success(self):
        ...
        self.assertTrue(
            Enrollment.objects.filter(student=self.student, course=self.published_course, is_active=True).exists()
        )
    ...
    def test_toggle_lesson_completion(self):
        ...
        self.assertTrue(
            UserProgress.objects.filter(student=self.student, lesson=self.lesson, is_completed=True).exists()
        )
    ...
    def test_classroom_aggregate_details(self):
        ...
        UserProgress.objects.create(
            student=self.student, course=self.published_course, lesson=self.lesson, is_completed=True
        )
        WatchHistory.objects.create(
            student=self.student, lesson=self.lesson, last_position=60, completion_percentage=10.0
        )
